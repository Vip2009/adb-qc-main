...
const { pairDeviceFromQR, pairDevice, connectToDevice,executeAdbCommand } = require('../utils/adbUtils');
...
async function connectToDeviceByIpPort(ipaddress) {
  //handleProcessInterruption();
  

  return new Promise((resolve, reject) => {
    console.log("connecting via port and ip")
    executeAdbCommand(
      ['connect', ipaddress],
      (stdout) => {
        const output = stdout.trim();
        if (output.startsWith('connected to')) {
          console.log(`Connected to device: `+ipaddress);
          resolve();
...
async function startDiscoverQr(name, password, { maxRetries = 30, retryDelay = 1000, autoConnect = false } = {}) {
  ...
  const ports =
  report['172.20.10.3'].host?.[0]
    .ports?.[0]
    .port
    ?.filter(p => p.state?.[0]?.item?.state === 'open')
    .map(p => Number(p.item.portid)) || [];

console.log(ports);
try{
    connectToDeviceByIpPort(device.address+":"+ports[0])
         ...
         if (false) await connectToPreviouslyPairedDevice();
         return device;
        } else {
...
module.exports = { discoverDevices, startDiscoverQr, startPairAndConnect, connectToPreviouslyPairedDevice,connectToDeviceByIpPort };